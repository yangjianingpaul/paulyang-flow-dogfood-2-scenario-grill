...
def test_create_book_returns_201_with_id_and_title(base_url, http):
    ...
    status, headers, body = http("POST", f"{base_url}/books", {"title": "三体"})

    assert status == 201
    assert headers["Content-Type"] == "application/json"
    assert body["title"] == "三体"
    assert isinstance(body["id"], str) and body["id"]
    assert set(body) == {"id", "title"}


def test_book_id_shape_is_bk_n_starting_at_1(base_url, http):
    """TC9: id 形态 "bk_<n>"，n 为从 1 起的进程内单调递增整数。"""
    _, _, first = http("POST", f"{base_url}/books", {"title": "三体"})
    _, _, second = http("POST", f"{base_url}/books", {"title": "球状闪电"})
...
def test_book_ids_are_unique_and_never_reused(base_url, http):
    ...
    ids = [
        http("POST", f"{base_url}/books", {"title": f"book-{i}"})[2]["id"]
        for i in range(5)
    ]
